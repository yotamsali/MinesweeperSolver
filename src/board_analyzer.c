...
#define UNKNOWN_MAP_CELL -1
#define KNOWN_MAP_CELL -2
#define FINAL_POSITIVE 1
#define FINAL_NEGATIVE 0
#define NEIGHBORS_NUMBER 8
#define RANDOM_MOVE_THRESHOLD 0.92
#define IS_NULL_MAP_VALUE(x) (x == UNKNOWN_MAP_CELL || x == KNOWN_MAP_CELL)
...
t_neighbors_data get_neighbors_data(t_ptr_board board, t_board_cell neighbor_cells[]) {
    int clear_counter = 0, unknown_counter = 0, mine_counter = 0;
...
t_move get_random_move(t_ptr_map clear_map) {
    t_move random_move = {{0, 0}, false};
    ...
    for (int i = 0; i < board_size._x; i++)
        for (int j = 0; j < board_size._y; j++) {
            ...
            if (GET_CELL(clear_map, cell) != KNOWN_MAP_CELL)
...
t_move select_optimal_move(t_ptr_map mine_map, t_ptr_map clear_map) {
    ...
    for (int i = 0; i < board_size._x; i++) {
        for (int j = 0; j < board_size._y; j++) {
            ...
            if (IS_NULL_MAP_VALUE(mine_probability) || IS_NULL_MAP_VALUE(clear_probability))
                continue;
    ...
    if (best_unsafe_probability < RANDOM_MOVE_THRESHOLD)
        return get_random_move(clear_map);
    t_move selected_unsafe_move = {best_unsafe_cell, is_best_unsafe_cell_mine};
    return selected_unsafe_move;
}
    ...
    for (int i = 0; i < board_size._x; i++)
        for (int j = 0; j < board_size._y; j++) {
            ...
            if (GET_CELL(board, cell) != UNKNOWN_CELL)
                ...
                SET_CELL(map, cell, UNKNOWN_MAP_CELL);
...
void set_neighbors_map_values(t_ptr_board board, t_ptr_map mine_map, t_ptr_map clear_map, t_board_cell cell,
    ...
    for (int k = 0; k < NEIGHBORS_NUMBER; k++)
        if (is_cell_in_board(neighbor_cells[k])) {
            ...
            update_map_cell(clear_map, neighbor_cells[k], mine_probability);
        }
}
